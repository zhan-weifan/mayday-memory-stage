...
export function loadApp(){
 if(!appPromise)appPromise=(async()=>{
  initShell();
  const ui=await import('./browser-inference/ui.js?v=mobile-20260922-2');
  await ui.ready;
  for(const path of ['./film.js?v=mobile-20260922-2','./observatory-ui.js?v=mobile-20260922-2'])import(path).catch(error=>showAppError(error,()=>import(path),'附加界面'));
  return ui;
...
void import('./viewer-loader.js?v=mobile-20260922-2').then(({ensureViewer})=>ensureViewer()).catch(()=>{});
