...
export async function ensureViewer(){
 ...
 if(!pending)pending=import('./main.js?v=repair-v2').then(module=>module.initViewer()).then(viewer=>{instance=viewer;if(ticket)viewer.setTicket(...ticket);return viewer;}).catch(error=>{pending=null;showAppError(error,async()=>{await ensureViewer();document.dispatchEvent(new Event('viewer-retry'));},'3D 查看器');throw error;});
 return pending;
}
