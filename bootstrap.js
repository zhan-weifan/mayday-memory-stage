...
export function loadApp(){
 if(!appPromise)appPromise=(async()=>{
  initShell();
  const ui=await import('./browser-inference/ui.js?v=low-performance-20260925-2');
  await ui.ready;
  for(const path of ['./film.js?v=low-performance-20260925-2','./observatory-ui.js?v=low-performance-20260925-2'])import(path).catch(error=>showAppError(error,()=>import(path),'附加界面'));
  return ui;
...
void import('./viewer-loader.js?v=low-performance-20260925-2').then(({ensureViewer})=>ensureViewer()).catch(()=>{});
