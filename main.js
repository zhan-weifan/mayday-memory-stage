...
import { MemoryGaussians } from './gaussian.js?v=repair-v2';
...
import {createViewerScope} from './viewer-scope.js';
import {constrainedDevice} from './device-capabilities.js';
import {normalizeSettings,CAMERA} from './settings.js';

let viewer=null;
export function initViewer(){if(!viewer)viewer=createViewer();return viewer;}
function createViewer(){
 const stage=document.getElementById('stage'),scope=createViewerScope(stage);
 try{
const $=id=>document.getElementById(id);

const verticalScroll=scope.node(document.createElement('div'));
...
scope.observe(verticalScroll,syncScroll);
scope.listen(verticalScroll,'scroll',()=>{
...
const scene=scope.own(new THREE.Scene());scene.background=new THREE.Color('#000000');
const inside=scope.own(new THREE.Scene());inside.background=new THREE.Color('#151713');
const studio=scope.own(createStudio(renderer));scene.environment=studio.environment;scene.environmentIntensity=.48;inside.environment=studio.environment;inside.environmentIntensity=.6;
...
const glassMaterial=scope.own(new THREE.ShaderMaterial({uniforms:{sharp:{value:innerRT.texture},soft:{value:blurB.texture},studioReflection:{value:studio.reflection},viewProjection:{value:new THREE.Matrix4()},resolution:{value:displaySize},frost:{value:.025},glow:{value:.12},time:{value:0}},vertexShader:`
...
const absHeight=scope.own(new THREE.TextureLoader().load('./baked/abs-height.png'));absHeight.colorSpace=THREE.NoColorSpace;absHeight.wrapS=absHeight.wrapT=THREE.RepeatWrapping;absHeight.anisotropy=8;
...
const ceremony=createCeremony(scene,inside,camera,stage);scope.cleanup(()=>ceremony.cancel());
...
const floorMaterial=scope.own(new THREE.MeshStandardMaterial({color:'#242a27',roughness:.72,metalness:0,envMapIntensity:.32}));
...
const floor=new THREE.Mesh(scope.own(new THREE.PlaneGeometry(2000,2000)),floorMaterial);floor.rotation.x=-Math.PI/2;floor.position.y=-2.59;floor.receiveShadow=true;scene.add(floor);
...
const composer=scope.own(new EffectComposer(renderer,scope.own(new THREE.WebGLRenderTarget(1,1,{type:THREE.HalfFloatType,samples:mobile?0:4}))));
composer.addPass(scope.own(new RenderPass(scene,camera)));
const ao=mobile?null:scope.own(new GTAOPass(scene,camera,1,1));
if(ao){
...
scope.listen(stage,'pointerdown',e=>{if(e.pointerType==='mouse'&&e.button!==0)return;cameraMove=null;pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});dragging=true;gestureDistance=span();stage.setPointerCapture(e.pointerId);});
scope.listen(stage,'pointermove',e=>{const old=pointers.get(e.pointerId);if(!old)return;const dx=e.clientX-old.x,dy=e.clientY-old.y;pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});if(pointers.size===1){azimuth-=dx*.006;elevation=Math.max(CAMERA.elevation.min,Math.min(CAMERA.elevation.max,elevation+dy*.004));}else if(pointers.size===2){const d=span();if(gestureDistance>5&&d>5)zoom=Math.max(CAMERA.zoom.min,Math.min(CAMERA.zoom.max,zoom*d/gestureDistance));gestureDistance=d;}setCamera();});
for(const ev of ['pointerup','pointercancel','lostpointercapture'])scope.listen(stage,ev,e=>{pointers.delete(e.pointerId);gestureDistance=span();dragging=pointers.size>0;});
scope.listen(window,'blur',()=>{pointers.clear();gestureDistance=0;dragging=false;});
scope.listen(stage,'wheel',e=>{e.preventDefault();cameraMove=null;zoom=Math.max(CAMERA.zoom.min,Math.min(CAMERA.zoom.max,zoom*Math.exp(-e.deltaY*.001)));setCamera();},{passive:false});
function fitContent(){
...
for(const id of ['glow','frost','brightness'])scope.listen($(id),'input',e=>{const n=Number(e.target.value);$(id+'Value').textContent=n.toFixed(2);if(id==='brightness'){if(memoryMesh)memoryMesh.material.uniforms.brightness.value=n;}else glassMaterial.uniforms[id].value=n;});
...
const memoryBox={
 dispose(){scope.dispose();viewer=null;},
 ...
 async beginFilm(w,h){if(filming||computing||ceremony.active||!memoryMesh)throw Error('请等待记忆加载或收藏动画完成后再制作影片');filmSaved={azimuth,elevation,zoom,time,fov:camera.fov};filming=true;cameraMove=null;renderer.setPixelRatio(1);composer.setPixelRatio(1);renderer.setSize(w,h,false);composer.setSize(w,h);displaySize.set(w,h);innerRT.setSize(w,h);blurA.setSize(w,h);blurB.setSize(w,h);camera.aspect=w/h;setCamera();filmDof=scope.own(new BokehPass(scene,camera,{focus:10,aperture:0,maxblur:.006}));filmDof.setSize(w,h);composer.passes.splice(composer.passes.length-1,0,filmDof);filmRevealed=false;memoryMesh.visible=false;tideAlpha=1;tideSwirl=0;const photo=await(await fetch(document.getElementById('photo-preview').src)).blob();await new Promise((resolve,reject)=>{ceremony.begin(photo,document.getElementById('memory-name').textContent,start=>{filmStart=start;resolve();}).catch(reject);});},
 ...
 waiting(message){if(memoryMesh)memoryMesh.visible=false;demo.visible=false;ceremony.wait(message);},
  ...
  async load(url,settings,reveal=false){const version=++loadVersion;const response=await fetch(url);if(!response.ok)throw new Error('无法读取 3D 记忆，请重试。');const buffer=await response.arrayBuffer();if(version!==loadVersion)return;settings=normalizeSettings(settings);const next=new MemoryGaussians(buffer);if(memoryMesh){inside.remove(memoryMesh);memoryMesh.dispose();}memoryMesh=next;inside.add(next);demo.visible=false;applySettings(settings,reveal);if(reveal&&ceremony.active)ceremony.reveal(next);},
...
scope.cleanup(()=>{if(window.__prismatic?.renderer===renderer)delete window.__prismatic;memoryMesh?.dispose();});
return memoryBox;
